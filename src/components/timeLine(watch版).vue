<template>
  <div class="box">
    ...
    -------------- 子组件中的双向绑定
    <!-- <p style="font-size: 40px; display: none">
      {{ doubleBindingData?.join(":") }}
    </p> -->
    --------------
  </div>
</template>
...
// 双向绑定
let doubleBindingData = defineModel(
  "timeArray",
  {
    set(v) {
      console.log('set');
...
const warning = () => {
  ...
  if(!doubleBindingData.value || doubleBindingData.value.length != 2) {
    throw new Error("输入数组长度需要是 2 项！");
  }
};
watch(
  () => doubleBindingData,
